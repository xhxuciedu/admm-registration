...
\]
The same block $B$ is used at every spatial location; this construction is not
a one-pixel spatial problem. The tensor factors commute, so the surrogate is
simultaneously diagonalizable. If $Bz_j=h_jz_j$ and
$G_0\phi_\xi=g(\xi)\phi_\xi$, its common modes are
...
$U_B(\rho,\alpha)$ denote the exact modal factor of that globally frozen
surrogate. The Cayley resolvent identity gives
\begin{equation}
...
$\theta_c(\rho)$.
\item For every remaining branch pair, form the intersection, relaxation-
transition, and stationary polynomials in Theorem~\ref{thm:finite}. Retain
...
predictor. FIRE and CIMA experiments show reduced iterations and
