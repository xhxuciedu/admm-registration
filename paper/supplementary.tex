...
are not used to choose the deployed four-corner parameters. For symmetric
...
can be evaluated by FFT in $O(n\log n)$ time and extremal comparison
...
$q_i\mapsto-q_i$. The archived controlled runs did not store a normalized
commutator or projector-distance summary, so no quantitative claim based on
such a statistic is made here.
...
$(U-\spr_{\rm act})/\spr_{\rm act}$ and the parameter-selection gap is
...
Perron rectangle retained sign information and had the smallest parameter gap
...
The ten $3\times3$ cases comprise periodic magnitude variation, smooth
orientation variation, one sharp orientation interface, and Gaussian-smoothed
image gradients; nine are smooth. The four-corner predictor attained median
parameter gaps $0.057$ overall and $0.027$ on the smooth subset, with no
collapse; the abrupt-interface gap was $0.193$. At the selected parameters,
the median absolute relative discrepancy between the four-corner envelope and
the true radius was $1.4\times10^{-15}$, with maximum
$2.8\times10^{-3}$. The Perron comparison certificate was
below one in all ten cases. Its median enclosure gap was $1.29$, establishing
validity while illustrating conservatism. A normalized commutator-magnitude
analysis remains TODO because that diagnostic was not stored by the archived
runs.
...
expensive $64^2$ retained-radius case took $81.4$ seconds on CPU. A relaxed
...
The external fixed pair $(\rho,\alpha)=(0.1,1)$ was selected using the
...
imbalance, and rescales the dual variable. The BB comparison is a safeguarded
Barzilai--Borwein penalty proxy with clipping and periodic updates. Fixed
oADMM uses $(1,1.8)$. Exact update intervals and safeguards are implemented in
...
single external fixed pair; held-out cases 3--5 were not used for tuning and
...
relative MSE change was $-0.0068$\%, and no tested case had a nonpositive
Jacobian.
...
initialization selected from phase-correlation, centroid, and clustered SIFT
proposals on a $128^2$ image.
...
tuning, median $\rho_{\rm 4C}=0.1008$ (IQR $0.0397$, range
$0.0688$--$0.1402$) and median $\alpha_{\rm 4C}=1.5699$ (IQR $0.1287$, range
...
the experiments were CPU-only. Timed registrations used one BLAS/OpenMP thread.
...
the structural-channel pipeline above. Supplied landmarks are used only for
evaluation.
